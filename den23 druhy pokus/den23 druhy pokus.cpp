...
void tah(std::vector<std::vector<char> > jamy, std::vector<char> riadok,int score) {
	if ((jamy[0].size() == 2) && (jamy[1].size() == 2) && (jamy[2].size() == 2) && (jamy[3].size() == 2)) {
	...
	for (size_t i = 0; i < riadok.size(); i++)
	{
		if (riadok[i] != 'e') {
			...
			if (i > riadok[i] - 95)
			{
				for (size_t j = riadok[i] - 95; j < i; j++)
				{
					if (riadok[j] != 'e')
					{
						volne = 0;
			...
			{
				for (size_t j = i + 1; j < riadok[i] - 95; j++)
				{
					if (riadok[j] != 'e')
					{
						volne = 0;
			...
			if (volne == 1)
			{
				...
				{
					if (riadok[i] != jamy[(riadok[i] - 97)][j])
					{
						vhodne = 0;
						break;
					}
				...
				{
					int i2 = i, pohyb =0;
					long double tahy = i2 - (riadok[i] - 97);
					pohyb = abs(tahy) + (2 - jamy[(riadok[i] - 97)].size());
...
int main(){
	using namespace std;
	vector<char> jamaa = { '0','b' }, jamab = { 'd','c' }, jamac = { '2','b' }, jamad = { 'a','d' }, riadok = { 'e','e','e','e','e','e','e','e','e','e','e' };
